...
export default function BorrowingPage() {
    ...
    useEffect(() => {
        loadBorrowings(page, search);
    }, [page, search]);

    const loadBorrowings = async (p = page, s = search) => {
        try {
            setLoading(true);
            const res = await getBorrowings(p, s);
    ...
    return (
        <div className="space-y-xl animate-in fade-in duration-500">
            ...
            <div className="bg-surface-container-low border border-outline-variant/30 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-md">
                <div className="p-4 border-b border-outline-variant/20 flex justify-end bg-surface-container/30">
                    <div className="relative w-72">
                        ...
                        <input
                            ...
                            className="w-full bg-surface-container-highest/50 border border-outline-variant/50 rounded-xl pl-9 pr-4 py-2 text-[11px] font-bold outline-none focus:border-primary transition-all text-on-surface placeholder:text-outline/50"
                        />
                    </div>
                </div>
                            ...
                            {loading ? (
                                ...
                                borrowings.map((b) => (
                                    <tr key={b.id} className="hover:bg-surface-bright/30 transition-colors group">
                                        <td className="p-5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className='flex flex-col'>
                                                    ...
                                                    <span className="font-mono text-[10px] text-outline">{b.contact}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-5">
                                        ...
                                        <td className="p-5">
                                            <span className="px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.1em] border bg-blue-500/10 text-blue-500 border-blue-500/20">
                                                {b.status}
                                            </span>
                                        </td>
                                        <td className="p-5">
